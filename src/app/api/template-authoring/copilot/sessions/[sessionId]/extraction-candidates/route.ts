...
export async function POST(request: NextRequest, context: { params: Promise<{ sessionId: string }> }) {
  ...
  catch (error) { const classified = classifyTemplateCopilotV2OperationError(error, "The extraction confirmation could not be completed."); return approvalJson(cookieSource, correlationId, { error: classified.error }, classified.status); }
}
