...
import { templateSimulationCommandSchema } from "@/lib/template-authoring-api-contracts";
import { simulateTemplateAuthoringDefinition } from "@/lib/template-authoring-validation";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = body.ok
    ? templateSimulationCommandSchema.safeParse(body.value)
    : null;
  if (!parsed?.success) {
    return approvalJson(
      cookieSource,
      correlationId,
      {
        error: {
          ...
          message: "The simulation payload is invalid.",
        },
      },
      400,
    );
  ...
  return approvalJson(cookieSource, correlationId, {
    simulation: simulateTemplateAuthoringDefinition(parsed.data),
  });
}
